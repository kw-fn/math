...
TEST(ErrorHandlingMatrix, isMatchingDims_compile_time_sizes) {
  ...
  EXPECT_FALSE_MSG(is_matching_dims<true>(m_dynamic, m_2x2));
  
  m_dynamic.resize(3, 3);
  EXPECT_FALSE_MSG(is_matching_dims(m_dynamic, m_2x2));

  m_dynamic.resize(4, 1);
  EXPECT_TRUE(is_matching_dims(m_dynamic, vector));

  EXPECT_FALSE_MSG(is_matching_dims<true>(m_dynamic, vector));
  
  m_dynamic.resize(3, 1);
  EXPECT_FALSE_MSG(is_matching_dims(m_dynamic, vector));

  m_dynamic.resize(1, 4);
  EXPECT_TRUE(is_matching_dims(m_dynamic, rowvector));
  
  EXPECT_FALSE_MSG(is_matching_dims<true>(m_dynamic, rowvector));

  m_dynamic.resize(1, 3);
  EXPECT_FALSE_MSG(is_matching_dims(m_dynamic, rowvector));
}
